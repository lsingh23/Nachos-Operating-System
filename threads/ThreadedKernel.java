...
public class ThreadedKernel extends Kernel {
    ...
    public void selfTest() {
        Semaphore.selfTest();
        SynchList.selfTest();
        if (Lib.test(KThread_TEST))
            KThread.selfTest();

        if (Lib.test(Condition2_TEST))
            Condition2.selfTest();

        if (Lib.test(Alarm_TEST))
            Alarm.selfTest();

        if (Lib.test(Communicator_TEST))
            Communicator.selfTest();

        if (Lib.test(React_TEST))
            ReactWater.selfTest();
    ...
    final static char KThread_TEST = '1';
    final static char Condition2_TEST = '2';
    final static char Alarm_TEST = '3';
    final static char Communicator_TEST = '4';
    final static char React_TEST = '5';
}
